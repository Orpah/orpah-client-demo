...
- **c4（进行中）**：把协议内核实接进主循环 —— §8.2 选级 + 无 RTC（`ts=0`/`cap.rtc=false`）
  ...
  · ★ **模组侧确认也过了（2026-09-22，用户修正接线后）**：COM24 上逐字拿到
    `[393206][mbus rx] 403 byte(s) 2b 1a 09 00 93 01 74 00 ff ff ff ff ff ff 4a 06 ...`
    —— `403 = 8 B HGIC 头 + 395 B 帧`；`type=0x09`（FRM2，与 2026-09-16 记录**同一形状**）、
    `len=0x0193=403`、`cookie=0x74=116`，载荷开头 = 以太帧（dst `ff:ff:ff:ff:ff:ff` 广播 + src MAC）。
    取证方式：`tools\check_c4g1_bench.py --mod-port COM24` **同时**抓两个口，脚本会挑这条日志打
    `PASS 模组侧确认：… [mbus rx] 403 byte(s)（= 8 B HGIC 头 + 395 B 帧）`。
    ⚠ **两条如实**：① 同一帧后面跟着一条模组警告 `cookie err: last:110, new:116` —— 本固件用
    **一个**计数器同时喂 CMD（每 3 s 探测）与 FRM2（`idsend`），而模组看起来**按数据通道**查顺序
    ⇒ 每个 FRM2 的 cookie 都比上一个 FRM2 大一截（110 → 116，中间隔着 5 条探测）。
    **没观察到代价**（帧照样 `[mbus rx]` 收下、长度正确），**是否影响上层未验证**；
    要试就把两个通道的计数器拆开 —— 按 §6「一次一改」，**先与用户对齐再动**。
    ② 「进了模组主机口」**≠**「上了空口」：AP 主机口 → 空口那段断裂是 b 步的既有结论
