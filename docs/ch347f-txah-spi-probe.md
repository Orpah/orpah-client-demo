...
**CH347F-EVT 这头的排针**（原厂手册 CH347DS1 第 13 页 + 板级原理图 CH347SCH）：

| 排针 | 内容 |
|---|---|
| **P4** | 4 线 SPI：`3.3V`、`GND`、**`SCS0`**、`SCK`、`MISO`、`MOSI`，外加可选的第 2 片选 `SCS1` |
| P5 | I2C（不接） |
| P6 | JTAG（接模组的 JTAG 才用） |
| P8 | 板载 **SPI FLASH 片选**（`SCS1/DTR1`、`SCS_FLASH`、`SCS0`）——**别拿来接模组**，那是给板载 flash 用的 |

片选：DLL 里 `CH347SPI_*` 的 **CS1 = `SCS0`（P4 上的主片选）**、**CS2 = `SCS1`（可选第二片选）**
—— 按手册「bit1/0 = 00/01 → CS1/CS2」+ `SCS0` 为主片的读法；**没有实测校验过**。
所以脚本默认 `--cs 1`（走 P4 的 SCS0）。

模组的 SPI 与 SDIO **共用管脚**（模组规格书接口表 / 《泰芯AH模组开发板使用说明》2 节）：

| 模组侧丝印 | 模组脚 | CH347F-EVT（P4） | 备注 |
|---|---|---|---|
| `SD_CLK` / `SPI_CLK1` | IOA6 | `SCK` | 原厂说明：需把 **R18/R20/R21/R23/R3/R25 焊 0R**（SPI 飞线） |
| `SD_CMD` / `SPI_MOSI1` | IOA7 | `MOSI` | 同上 |
| `SD_D0` / `SPI_MISO1` | IOA8 | `MISO` | 同上 |
| `SD_D1` / `SPI_INTIO1` | IOA9 | （本轮不接；可后续用 CH347 GPIO 观测中断） | |
| `SD_D3` / `CS1` | IOA11 | `SCS0`（脚本默认 `--cs 1`） | 同上 |
| GND | GND | `GND` | **必须共地** |

- 供电：模组 VCC **3.1~3.3 V（别接 5 V）**。**建议用模组开发板自己的供电**（按说明选
  `VCC`/`SVCC` 跳帽），**只共地 + 接 SPI 4 根线** —— CH347F-EVT 上那点 3.3V 是给逻辑用的，
  模组发射瞬间的电流它多半兜不住（会表现为"时好时坏"，最坑）。
- ⚠ 该组脚与 **TF 卡座**共用 —— 探测前先确认没有别的东西在驱动这几根线（拔掉 TF 卡、断开那一路）。
