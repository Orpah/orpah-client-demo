...
| 地 | `CH347F.GND` ↔ `ATECC608B.GND`(pin4 @E4) |
...
- ⚠ **取脚位置**（电气上都不是错，但接线时要跨排针，且有一条要你确认）：
  1. 地取的是 **`JP1` 那列的 GND 脚**（不是 P5 第 3 脚的 GND）；3V3 取的是 **P2 排针末端的 3V3**。
     板内 GND 共地、3V3 同一轨 ⇒ **电气等价**，只是按图接线会用到三个排针。
  2. `JP1` 是 VIO 电平跳线块（丝印上→下 = `GND/VIO/VIO/3V3`）⇒ 那一块上**如果有跳线帽，别和我们的地线搞在一起**。
  3. **P5 第 4 脚的丝印是 `VIO`（不是 3V3）**，图上**没接**；而我们的 CH347F 部件生成脚本注释里写的是
     "P5 = SDA/SCL/GND/3V3" —— **两者不一致，以丝印为准**。⇒ 上机前请确认 `JP1` 已把 **VIO 跳到 3V3**
