...
C:\Python313\python.exe tools\ch347_i2c.py health --n 10         # 总线健康度：读正对照 0x50 ×10
...
| 命令 | 看什么 | 含义 |
|---|---|---|
| **`health --n 10`** | 读正对照 `0x50` 的**成功率**（退出码 0 = ≥95%） | **≈100% = 总线干净**；明显偏低 ⇒ 有东西在拖总线（器件/焊接/接触）—— 这是**先于一切器件判据**的健康度指标 |
...
- **2026-09-23 14:0x 追 `0x64`（当天下午的第二轮，含一次自我纠错）**：
  - `0x64` **确实有器件在 ACK**：写方向常 ACK（有时两向都 ACK）；对**不存在的地址**
    `probe` 干净 NACK、读**直接报错**（不返回垃圾字节）⇒ "读回非 FF"在这一点上不含糊。
  - **写命令 60/60 全部 ACK**（`Info`/`Random` 都收下）；**读响应 37/60 成功，但每次读回来
    逐字节相同**：`04 FF 01 42 FF FF…`（与命令无关）；不发命令直接读则是 `04 11 33 43 FF…`；
    **CRC 一次都没对上**。
  - **用户换了一颗芯片后，这套数字一个都没变** ⇒ **这 4 个字节到底是芯片驱动的还是夹具/DLL
    自己冒出来的，仍未判定** —— 要判它，得做"**把 SE 整块拔下来**再读同一个地址"的对照（**未做**）。
  - ★ **自我纠错（教训）**：这一轮的"时好时坏"里**有一部分是我自己的工装造成的** ——
    一个后台脚本（`se_try`，14:01 起）一直在轮询，与随后的命令行/守候工装**两个进程同时操作
    同一个 CH347**。同一时刻**只能有一个进程**用它（`CH347OpenDevice` 不互斥）⇒
    以后起后台工装前先 `Get-CimInstance Win32_Process -Filter "Name='python.exe'"` 查一遍。
    **杀掉残留进程后重测**：单进程下仍然会在**合写读事务**上偶发失败（1 字节地址探测几乎不失败）
    ⇒ 夹具脆弱性**不全是**进程打架，但两者的现象无法区分，**已受影响的单次观测一律不算数**。
  - 夹具可靠率实测（同一操作重复 20 次）：一瞬间 **20/20**，另一瞬间 **5/20**；20 kHz 一档 16/20、
    100 kHz 与 400 kHz 各 20/20 ⇒ **间歇性**，且"重新初始化 I²C"往往能立刻恢复。
  - **物理量测（用户 2026-09-23 给的）**：SE `pin8↔pin4` = **3.29 V 且稳定** ⇒ **SE 确实有供电**
    （"没供电"这一条被排除）；`pin5/pin6` 到 `P5` 第 1/2 脚的通断**量不了**（现场条件不允许）。
  - ⚠ **取电点变过**：3V3 先接 **P2 末端**，现在是 **`P5` 第 4 脚 —— 丝印 `VIO`**。`VIO` 是
    **I/O 参考电平**脚（不是给负载供电的输出），而两只 4.7 kΩ 上拉在数据期各要 ~1.4 mA
    ⇒ 从 `VIO` 取会被拖动（**它能不能解释"ACK 时有时无、数据期常失败"：未验证**）。
    下次**先把它改回 P2 末端的 3V3**（成本 10 秒，是目前唯一没排除的供电变量）。
  - **未做（卡住整条推理的一步）**：把 SE **整块拔下来**再读 `0x64` 的对照 ⇒
    **`0x64` 到底是不是 SE，仍未判定**（换了一颗芯片后数字逐字节不变，反而不支持"它就是 SE"）。
## 5. 未做 / 边界（如实）
...
- ★ **下次（立创那 5 颗到了）的固定流程**（当天就按这个走，不要再临时改）：
  ① 3V3 接 **P2 末端**（**不要**用 `P5` 的 `VIO`），先确认 SE `pin8↔pin4` ≈3.3 V；
  ② 先跑 **`health`**（读正对照 `0x50` ×10）：**≈100% 才算总线干净**；
     插上 SE 后健康度一揀下去 ⇒ 就是它/它的焊接/接触在拖总线，先处理硬件；
  ③ `scan` 看 ACK（`0x50` 必须稳定在线 = 正对照）；
  ④ 上电窗口守候 + `atecc … --op random` 多试（失败即重初始化）；
  ⑤ 对拍：`count` 合法（8/10/36/38）+ **CRC 对上**才算"器件回了响应"；
  ⑥ **每步只允许一个进程**碰 CH347（先查 `python.exe` 残留）。
- ★★ **2026-09-23 14:20 的对照（决定性）**：把 SE **整块从面包板上取下来**后，单进程 5 遍全部：
  · 扫描**只剩 `0x50`**，`0x64` **消失** ⇒ **`0x64` 就是那颗 SE（已钉死）**；
  · 读 `0x64` 5/5 失败（与"对不存在地址读会报错"的木底一致）；
  · **读正对照 `0x50` 从（SE 在线时的）~50% 成功率恢复成 5/5 全成功**。
  ⇒ **SE 一挂上就把整条总线拖坏**（连板载 EEPROM 都跟着失败）—— 这是**电气层**的故障
  （焊接/接触/器件），不是协议层；当天下午"时好时坏"的主要成因就是它（工装并发是次要污染源）。
  下一步（硬件）优先顺序：**先把 8 个焊点补焊/检查**（SOIC-8→DIP 转接板的典型故障），
  再重插到**干净列**，然后用 `health` 当插拔前后的"总线健康度"指标。
