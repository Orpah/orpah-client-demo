...
⇒ 剩下的疑点只有 **`P2 → A10/A11` 这一段**（板上落点 J5 pin3/pin5）。**分层排查法**（都借已知好信号，
不用烙铁）：把 P2 的 `RXD0` 临时接到模组的**打印输出脚**（`J4` 的 pin 2 或 pin 3，两者都试一下，
因为 A12/A13 谁是 TX 在两侧资料里说法不一致）→ `--ch347-com 0 listen` **能看到打印文字**就证明
"P2 的 RX 路 + COM23 + 这根线"都好；再反过来把 `TXD0` 接到模组的 RX 脚、从 COM23 发
`AT+VERSION=?`、在 COM24 上看有没有回应，即可证明 TX 路也好。

若两边都证明好、而 `probe` 仍无回应 ⇒ 下一步是**给固件加 RX 调试打印**（在
