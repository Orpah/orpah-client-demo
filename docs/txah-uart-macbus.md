...
| **cookie** | `hgic.h`：`HGIC_TX_COOKIE_MASK 0x7FFF`、`HGIC_TX_WINDOW 20`、`HGIC_BLOCK_ACK_CNT 256`。模组对 cookie 做**顺序检查**，跳变就打印 `cookie err: last:<上次>, new:<本次>`（2026-09-16 真机实测：`last:167, new:177`）。实现：`tools/txah_hgic.py` 的 `CookieCounter`；`tools/probe_txah_uart.py` 每帧都用它（`--cookie` = 起始值，默认 1；`--cookie-fixed N` 钉死复现旧行为；`--count N` 连发 N 帧、cookie 连续）。<br>★ **2026-09-22 修正：顺序是「按通道」查的，不是「全局逐帧 +1」** —— 模组只拿**上一条数据帧（`FRM2`）**的 cookie 跟这条比，命令帧（`CMD` 探测）不参与。证据（固件侧对照实验，重刷后连发两条 `idsend`）：第 1 条 FRM2 cookie `01 00` → `cookie err: last:271, new:1`（模组的 last 停在**复位前**的共享计数值）；第 2 条 FRM2 cookie `02 00` → **不再报**；同期 CMD 探测 `11 00`/`12 00` 模组一句都不抱怨。⇒ 以前 CMD 与 `FRM2` **共用一个**计数器时，每条数据帧都必然显得"跳了一截"。固件现已拆成两个计数器（`Periph/hgic_uart.c` 文件头 ★3）。<br>⚠ 剩下的边界：**每次复位后第一条** `FRM2` 必然报一次（cookie 从头开始、模组记着旧的大值；固件无 NVM 保存该计数）—— 实测**没观察到后果**（那条帧照样 `[mbus rx] 403 byte(s)` 被收下），但「会不会因这次 err 丢帧」**未验证**（要证得看它是否上空口，而主机口→空口那段本就没通，见 `hardware/wiring/README.md`）。 |
...
5. **CH347F 的 VCP 会周期性抽风**（I/O 报 `PermissionError(13, '拒绝访问')`；另有程序占着端口同样如此）⇒
   ...
   ⚠ 还有一种更容易误判的表现：**整个 CH347F 从设备管理器里消失**（板子红 LED 有电、但 Windows
   里连报错设备都没有）——2026-09-20 实测原因是**PC 那个 USB 口不稳**，**换一个 USB 口即恢复**
   ⇒ 遇到先换口/换线，别先怀疑驱动或板子。
6. **AP 的 per-STA `rx1_cnt`/`tx1_cnt` 是分区间快照，不能当"过没过空口"的判据**（2026-09-20 实测：
   同一状态连续两次读回 `5 → 5`，而中间明明发过帧且对端回过应答）。
   ⇒ 判"上/下行真的过了空口"只有一条可靠办法：**发一条对端必然会应答的帧（如 DHCP DISCOVER），
   收到应答即反证双向通路** —— `tools\demo_client_hgic.py` 就是这么判的。
7. **TH-RJ45 上有拨码开关**：拨到 README 说的 AP 那一档（用户看是 **ON** 那侧）+ 断电重上电后，
   `AT+WIFIMODE=?` **开机就是 `ap`**（2026-09-20 对照：不拨时开机回 `sta`，拨了之后开机就是 `ap`）。
   ⇒ 角色是**开关定的**，不必每次敲命令。⚠ 开关确切语义未查原厂资料（README 只写 AP 拨 `NO`、STA 拨 `1`），
   **以读回 `AT+WIFIMODE=?` 为准**；也**不要按 Connect 配对键**（靠同 SSID 就能连上）。
