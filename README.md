...
> **现状（2026-09-21）：自家固件已在真机上跑通** —— `firmware/`（CH32V203）带控制台、1 ms 时基、
> 心跳灯与**模组数据口**（USART2 ↔ TX-AH 的 HGIC 双向通）；
> 台架接线/判据/四个真凶见 `docs/c3-2b-bench-bringup.md`。
> 协议内核（`proto/`）已与 Python 参考零偏差；选级 / 无 RTC / 自限频 / 已签上报（c4）**未上机**。
> （更早的口径：2026-09-14 时本仓只有规则、骨架与文档。）
> 唯一跑通过的真机环节是 **b 步的串口测试夹具**（nanoCH32V203 当 USB-UART 桥 → `COM32`
> 通到 TX-AH 的 AT 口、`AT+SSID?` 有回应）：见 `docs/nano-ch32v203-uart-bridge.md`。
> **数据面（`AT+TXDATA`/`FRAME:RX`）与全部固件仍是「未验证」** —— 任何"已支持/已验证"
> 的说法在目前都不成立。
...
| 文档 | 内容 |
|---|---|
| `docs/nano-ch32v203-uart-bridge.md` | b 步真机夹具：nanoCH32V203 刷 `SimulateCDC` 当 USB-UART 桥（COM32）、接线表、AT 验证、未做项 |
