...
+ **面包板同列导通**），再与脚本里的**期望表**逐网比对 —— 期望表就是这几套夹具的接线规格（单一源）。
**2026-09-23 实跑：6 张图均「全部对上 ✓」（退出码 0）；改图时先改 `EXPECT`。**
...
- 图里那块绿转接板旁边有一行蓝字 **`ATECC608B`** + 一小段蓝引线指向它（板上芯片本身只有 `CN`
  一个丝印）⇒ 看图先认「蓝字 + 蓝引线」那一块。两只上拉**没画阻值字**，值看上面表里的 `4.7 kΩ`。
  · 这行蓝字与引线是**放在 `.fzz` 工程里**的：挂在面包板那个「logo 图片」项
    （`newBreadboardLogoImageModuleID`）的 `shape` 属性上 ⇒ **重新导出不会丢**。
    ⚠ 别在 Fritzing 里删掉/换掉那个 logo 项，否则这行字会一起没。
  · 它是**描边字**（源 SVG 里 `fill="none"` + 蓝描边）⇒ 笔画看着比别的标注细；要实心就把那段
    `shape` 的 `fill` 改成 `#0000ff`（或在源 SVG 里给字填色）再重新加载。
- ⚠ 图上那几段**橙色说明文字**（`供电` / `刷……固件` / `打印日志……` / `数字签名`）**不在 `.fzz` 里**
  —— 把整个 `.fzz`（`.fz` + 4 个 `.fzp` + 16 个视图 svg）逐字节查过，**一个中文字都没有**
  （`.fz` 的非 ASCII 字符只有一个 `±`），它们是贴在**导出的 `.svg`** 上的 ⇒ 严格说重新导出会丢
  （要长期保留，就跟 `ATECC608B` 一样也放进工程）。
...
拿到了 = **我们固件还有毛病**。逐网核对表与判据表在
[`../../docs/ch347f-i2c-crosscheck.md`](../../docs/ch347f-i2c-crosscheck.md) §2/§2b（**单一源**，这里不再抄一份）。

三条**接线时留意**：
- 两个主机不能同时驱动同一对线 ⇒ **nano 断电或按住 RST**（这块图上不出现 nano 就是这个意思）；
- 图上 **SE + 两只 4.7 kΩ 吃 CH347F 的 3V3**（与 c4-γ-2 那张相反：那张吃 nano 的 3V3）；
- `P5` 第 4 脚丝印是 **`VIO`**，图上没接 ⇒ 上机前确认 **`JP1` 已把 VIO 跳到 3V3**（**未核实**）。
