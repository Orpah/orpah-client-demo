...
| c4 | §8.2 选级 + 无 RTC（`ts=0`/`cap.rtc=false`）+ 自限频 + 已签 ID 上报 | **内核已完成（2026-09-22）**：降级（HS256）→ 服务端验签通过（c4-α）；ES256 曲线/ECDSA/RFC 6979（c4-β-1/2）；**level=0 已接进 `idr_build()` 并过上游 `verify_report`**（c4-β-3：交叉测试 14 组 exit 0、`id-report-selftest` 7/7、服务端接受 6 条 level=0/1/2）。**c4-γ-1（2026-09-22）已接进固件主循环且★上机内容判据已过**（`tools/check_c4g1_bench.py`：`AT`→`OK` + 帧 395 B + 上游接受 level=0/trust=high；实测一条 **build_ms≈9.1 s**，即 8 MHz 上 ES256 的代价）。**c4-γ-2（2026-09-22）代码已就位、PC 侧判据已过**：nonce 换 **ATECC608B `Random(0x1B)`**（`Periph/i2c.c` 硬件 I2C1 @ `PB6/PB7` + `Periph/atecc.c` 唤醒/事务/自检 + `proto/crc16.c`/`atecc_msg.c` 纯函数层 + `id_build` 的 **nonce provider 注入点**）；交叉测试 **16 组** exit 0（新增 `crc-selftest` 7 行 / `msg-selftest` 9 行零偏差，含**外部校验值** CRC-16/BUYPASS("123456789")=`0xFEE8`）。**剩**：上机（控制台 `se` / `id` 的 `nonce_src=se`）+ 两条待实测确认的假设（线上 CRC 模式、响应 count 36/38，见 `docs/atecc608b-se.md`） |
| c5 | 低功耗 / 取能标定 | 推后到 d/e |
